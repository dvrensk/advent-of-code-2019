class Manhattan
  ...
  class Wire
    ...
    def draw
      @positions = []
      x = y = 0
      @path.split(",").each do |vector|
        direction, count = vector.match(/(.)(.+)/)[1,2]
        count.to_i.times do
          case direction
          when "R" then x += 1
          when "L" then x -= 1
